			...
			<td colspan="4">
              ...
              <?php if(!isset($this->options['settings']["feeds_api_key"])): ?>
                <p class="description"><?php _e('Let our editorial department and practice development tools work for you. - ', "rss_pi"); ?> 
                  <a href="<?php _e(SENDERLAW_ELA_ATTORNEY_SIGNUP_URL); ?>" target="_blank"> Try it today!</a>
                </p>
              <?php endif; ?>
						</td>
						<td>
							<?php $feeds_api_key = isset($this->options['settings']["feeds_api_key"]) ? $this->options['settings']["feeds_api_key"] : ""; ?>
							<input type="text" name="feeds_api_key" id="feeds_api_key" value="<?php _e($feeds_api_key); ?>" />
						</td>
					</tr>
						...
						<td>
							...
							<select name="post_status" id="post_status">
								...
								foreach ($statuses as $status) {
									?>
									<option value="<?php _e($status->name); ?>" <?php
									if ($this->options['settings']['post_status'] == $status->name) : _e('selected="selected"');
									endif;
									?>><?php _e($status->label); ?></option>
											<?php
										}
										?>
							</select>
