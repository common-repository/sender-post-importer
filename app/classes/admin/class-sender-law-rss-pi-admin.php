...
class senderLawRssPIAdmin {
  ...
  public function __construct() {
    ...
    // initialise logging
    $this->log = new \SenderLaw\senderLawRssPILog();
    $this->log->init();

    // load the form processor
    $this->processor = new \SenderLaw\senderLawRssPIAdminProcessor();
  }
  ...
  public function ela_rss_template_include($template) {
    ...
    elseif($import_page === 'role-update') {
      ...
      if ($new_role === 'triple' && $current_role === 'asnp') {
        ...
        $this->options['settings']['estate_post_status'] = $this->options['settings']['asnp_post_status'];
        $this->options['settings']['estate_category'] = $this->options['settings']['asnp_category'];
        $this->options['settings']['estate_author_id'] = $this->options['settings']['asnp_author_id'];
        $this->options['settings']['estate_allow_comments'] = $this->options['settings']['asnp_allow_comments'];
      } elseif ($new_role === 'triple' && $current_role === 'ela') {
        ...
        $this->options['settings']['estate_post_status'] = $this->options['settings']['ela_post_status'];
        $this->options['settings']['estate_category'] = $this->options['settings']['ela_category'];
        $this->options['settings']['estate_author_id'] = $this->options['settings']['ela_author_id'];
        $this->options['settings']['estate_allow_comments'] = $this->options['settings']['ela_allow_comments'];
      } elseif ($new_role === 'estateEla' && $current_role === 'ela') {
        $this->options['settings']['estate_post_status'] = $this->options['settings']['ela_post_status'];
        $this->options['settings']['estate_category'] = $this->options['settings']['ela_category'];
        $this->options['settings']['estate_author_id'] = $this->options['settings']['ela_author_id'];
        $this->options['settings']['estate_allow_comments'] = $this->options['settings']['ela_allow_comments'];
      } elseif ($new_role === 'dual' && $current_role === 'asnp') {
        $this->options['settings']['ela_post_status'] = $this->options['settings']['asnp_post_status'];
        $this->options['settings']['ela_category'] = $this->options['settings']['asnp_category'];
        $this->options['settings']['ela_author_id'] = $this->options['settings']['asnp_author_id'];
        $this->options['settings']['ela_allow_comments'] = $this->options['settings']['asnp_allow_comments'];
  ...
  function ela_rss_admin_menu() {
    add_options_page('Sender Post Importer', 'Sender Post Importer', 'manage_options', 'rss_pi', array($this, 'screen'));
  }
  ...
  public function ela_rss_enqueue($hook) {
    ...
    wp_enqueue_style('rss-pi', SENDERLAW_RSS_PI_URL . 'app/assets/css/style.css', array(), SENDERLAW_ELA_RSS_PI_VERSION);
    wp_enqueue_style('rss-pi-jquery-ui-css', SENDERLAW_RSS_PI_URL . 'app/assets/css/jquery-ui.css', array(), SENDERLAW_ELA_RSS_PI_VERSION);
    ...
    wp_enqueue_script('rss-pi', SENDERLAW_RSS_PI_URL . 'app/assets/js/main.js', array('jquery'), SENDERLAW_ELA_RSS_PI_VERSION);

    // localise ajaxuel for use
    $localise_args = array(
      'ajaxurl' => admin_url('admin-ajax.php'),
      'pluginurl' => SENDERLAW_RSS_PI_URL
    );
  ...
  function screen() {
    ...
    include( SENDERLAW_RSS_PI_PATH . 'app/templates/admin-ui.php');
  }
  ...
  function ajax_import() {
    ...
    update_option('rss_pi_feeds', array(
      'feeds' => $this->options['feeds'],
      'settings' => $this->options['settings'],
      'latest_import' => current_time('Y-m-d H:i:s'),
      'imports' => $imports
    ));
    ...
    // log this
    \SenderLaw\senderLawRssPILog::log($post_count);
