...
class senderLawRssPIEngine {
  ...
  private function _import($args = array(), $ids) {
    ...
    $defaults = array(
      'feed_title' => 'Sender.Law',
      'max_posts' => 100,
      'estate_author_id' => 1,
      'estate_category_id' => 0,
      'tags_id' => array(),
      'keywords' => array(),
      'strip_html' => false,
      'save_to_db' => true,
  ...
  private function url($ids = '') {
    ...
    $url = SENDERLAW_ELA_RSS_PI_BASE_URL . 'token/' . urlencode($key) . '/ids/' . urlencode($ids) . '/version/' . urlencode(SENDERLAW_ELA_RSS_PI_VERSION);
  ...
  private function insert($items, $args = array(), $newsletter_import) {
    ...
    $parser = new \SenderLaw\senderLawRssPIParser($this->options);
    // Featured Image setter
    $thumbnail = new \SenderLaw\senderLawRssPIFeaturedImage();
    ...
    foreach ($items as $item) {
      if (!$this->post_exists($item, $newsletter_import)) {
        ...
        $catsEvolve = $item->get_categories();
        $orgsEvolve = $catsEvolve[0];
        $organization = $orgsEvolve->get_term();

        //Estate Post
        if($organization === 'ESTATE') {
          $post = array(
            ...
            'post_status' => $newsletter_import ? 'publish' : $this->options['settings']['estate_post_status'],
            'post_author' => $this->options['settings']['estate_author_id'],
            'post_category' => array($this->options['settings']['estate_category']),
            'post_name' => preg_replace('/-\d+$/', '', end(explode('/', $item->get_link()))),
            'tags_input' => $tags_name,
            'comment_status' => $this->options['settings']['estate_allow_comments'],
